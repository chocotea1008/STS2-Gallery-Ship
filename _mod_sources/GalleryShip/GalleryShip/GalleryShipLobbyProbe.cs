...
internal static class GalleryShipLobbyProbe
{
	...
	private static ClientLobbyJoinRequestMessage CreateJoinRequest()
	{
		...
		try
		{
			maxAscension = Math.Max(maxAscension, saveManager.Progress.MaxMultiplayerAscension);
		}
		catch
		{
			// Fall back to the fixed floor if the live progress value is unavailable.
		}
	...
	private sealed class ProbeDisconnectedException(NetErrorInfo info) : Exception
	{
